...
const StreamSession = ({
  ...
  const streamValue = useTypedStream({
    ...
    onError: (error) => {
      ...
      if (msg?.includes("Thread with ID") && msg?.includes("not found")) {
        setThreadId(null);
      }
    },
        ...
        toast.error("Não foi possível conectar ao assistente.", {
          description: () => (
            <p>
              ...
              <code>{apiUrl}</code> e que sua chave de API está correta.
            </p>
          ),
          action: {
...
const DEFAULT_ASSISTANT_ID = "agendai_agent";
...
export const StreamProvider: React.FC<{ children: ReactNode }> = ({
  ...
  // For API key, use localStorage → NEXT_PUBLIC_LANGGRAPH_API_KEY → empty
  const [apiKey, _setApiKey] = useState(() => {
    const storedKey = getApiKey();
    return storedKey || process.env.NEXT_PUBLIC_LANGGRAPH_API_KEY || "";
  });
